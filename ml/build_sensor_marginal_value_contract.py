...
def main() -> None:
    ...
    contract = {
        ...
        "sign_convention": {
            # Audit M1/§19: the favorable direction is METRIC-SPECIFIC, not a universal
            # subtraction rule. Every record must carry metric_directionality so a
            # higher-is-better metric (e.g. macro-F1 for Sleep-EDF classification, recorded
            # elsewhere) is never silently inverted by a lower-is-better formula.
            "lower_is_better_absolute_benefit": "baseline_metric - candidate_metric (positive = candidate improves; used for MAE/RMSE)",
            "higher_is_better_absolute_benefit": "candidate_metric - baseline_metric (positive = candidate improves; used for macro-F1 and other higher-is-better metrics)",
            "relative_improvement": "signed_absolute_benefit / baseline_metric, as a fraction (positive = candidate improves), using the metric-appropriate absolute_benefit above",
            "favorable_direction_rule": "favorable_direction is determined per record by metric_directionality; there is NO universal implicit subtraction.",
            "scope_note": "Every experiment record in THIS contract is lower-is-better (MAE/RMSE); the formula must be inverted for higher-is-better metrics used by classification targets recorded in other artifacts.",
        },
        "metric_policy": {
        ...
        "interaction_effects": {
            ...
            # Audit M20/§20: an interaction experiment HAS now been run (Day-10). Do not
            # imply zero interaction evidence or global factorial coverage.
            "note": "Value(component_A + component_B) is not assumed equal to Value(A) + Value(B). One controlled interaction pair has been tested (Day-10 Sleep-EDF EEG x EOG x Resp, capacity-fair, 5 seeds); its interaction term was approximately additive / unresolved. This is NOT global factorial evidence across all candidate pairs.",
            "interaction_evidence": "partial_one_controlled_pair",
            "controlled_pairs_tested": 1,
            "global_interaction_coverage": "partial",
            "interpretation": "approximately_additive_or_unresolved",
            "interaction_experiment_artifact": "results/sleep_edf_interaction_resp_day10.json",
            "formal_limitation_doc": "docs/SENSOR_INTERACTION_LIMITATION.md",
            "consequence": "One-at-a-time marginal-value evidence in this contract, plus one partial interaction pair, does NOT establish a globally minimal sensor subset for any target.",
        },
