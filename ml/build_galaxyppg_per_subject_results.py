...
RESULT_PATH = REPO_ROOT / "results" / "galaxyppg_hr_external_replication_stage2.json"
...
def main() -> None:
    ...
    for seed in SEEDS:
        ...
        model_a.load_state_dict(torch.load(CKPT_DIR / f"galaxyppg_hr_A_cap_seed{seed}.pt", map_location="cpu"))
        model_a.eval()
        model_b = PPGPlusIMUModel()
        model_b.load_state_dict(torch.load(CKPT_DIR / f"galaxyppg_hr_B_seed{seed}.pt", map_location="cpu"))
        model_b.eval()
        model_c = PPGPlusIMUModel()
        model_c.load_state_dict(torch.load(CKPT_DIR / f"galaxyppg_hr_C_seed{seed}.pt", map_location="cpu"))
        model_c.eval()
    ...
    out_path = REPO_ROOT / "results" / "galaxyppg_hr_per_subject_stage2.json"
