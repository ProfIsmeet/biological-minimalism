...
SPLIT_PATH = REPO_ROOT / "results" / "hmc_split_stage2.json"
PROTOCOL_PATH = REPO_ROOT / "results" / "hmc_protocol_stage1b.json"
OUT_PATH = REPO_ROOT / "results" / "hmc_sleep_external_replication_stage2.json"
...
def main() -> None:
    ...
    out: dict = {
        "experiment_id": "hmc_sleep_external_replication_stage2",
    ...
    for name in ("A_eeg_only", "B_eeg_plus_eog"):
        ...
        for seed in SEEDS:
            ...
            ckpt_path = CKPT_DIR / f"hmc_sleep_{name}_seedfix_v2_seed{seed}.pt"
    ...
    for seed in SEEDS:
        ...
        ckpt_path = CKPT_DIR / f"hmc_sleep_c_shuffled_eog_seedfix_v2_seed{seed}.pt"
        torch.save(model.state_dict(), ckpt_path)
        ckpt_bytes = ckpt_path.read_bytes()
        out["checkpoint_manifest"].append({
