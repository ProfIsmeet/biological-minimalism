...
SPLIT_PATH = REPO_ROOT / "results" / "hmc_split_stage3_bounded_n7.json"
PROTOCOL_PATH = REPO_ROOT / "results" / "hmc_protocol_stage1b.json"
OUT_PATH = REPO_ROOT / "results" / "hmc_sleep_external_replication_stage3_bounded_n7.json"
...
def main() -> None:
    ...
    out: dict = {
        "experiment_id": "hmc_sleep_external_replication_stage3_BOUNDED_DIAGNOSTIC_n7",
        "scope_disclosure": "BOUNDED DIAGNOSTIC on 7 recordings (train=5/val=1/test=1) - PhysioNet's TLS certificate expired mid-download this sprint (docs/HMC_FULL_COHORT_CERT_EXPIRY_BLOCKER.md), stopping the full 151-recording download at 8 recordings (7 with complete EDF+scoring pairs). This is NOT the canonical full-cohort HMC replication result.",
        "frozen_protocol_source": "results/hmc_protocol_stage1b.json (Stage 1B commit fe32384, read-only, not merged)",
        "reduced_cohort_deviation_doc": "docs/HMC_FULL_COHORT_CERT_EXPIRY_BLOCKER.md",
        "split_source": "results/hmc_split_stage3_bounded_n7.json",
        ...
        "reduced_cohort_size": split_doc["reduced_cohort_size"],
        "full_frozen_cohort_size": split_doc["full_frozen_cohort_size"],
        "frozen_protocol": {
    ...
    for name in ("A_eeg_only", "B_eeg_plus_eog"):
        ...
        for seed in SEEDS:
            ...
            ckpt_path = CKPT_DIR / f"hmc_bounded_n7_{name}_seedfix_v2_seed{seed}.pt"
    ...
    for seed in SEEDS:
        ...
        ckpt_path = CKPT_DIR / f"hmc_bounded_n7_c_shuffled_eog_seedfix_v2_seed{seed}.pt"
        torch.save(model.state_dict(), ckpt_path)
        ckpt_bytes = ckpt_path.read_bytes()
        out["checkpoint_manifest"].append({
