#!/usr/bin/env python
"""GalaxyPPG full 6-fold grouped external replication (Stage 3, Priority 1).

Every one of the 24 real, eligible participants is held out as a test
subject exactly once, across 6 folds of 4 subjects each
(results/galaxyppg_full_cv_folds_stage3.json - frozen via the identical
random.Random(42) shuffle already used for the prior sprint's single-fold
diagnostic, before any full-CV training).

Fold 5 (test=P01/P04/P09/P21) is IDENTICAL in every respect (train/val/test
membership) to the prior sprint's single-fold diagnostic
(results/galaxyppg_hr_external_replication_stage2.json) - confirmed by
direct set comparison - so it is reused rather than retrained (avoiding
duplicate compute for an already-completed, byte-identical experiment).
This script trains folds 0-4 only.

Reuses the exact model classes, capacity-matching design, RNG discipline,
and windowing pipeline from
ml/train_galaxyppg_hr_external_replication.py (Priority 1 of the prior
sprint) - only the fold/split loop is new.
"""
...
FOLDS_PATH = REPO_ROOT / "results" / "galaxyppg_full_cv_folds_stage3.json"
SINGLE_FOLD_RESULT_PATH = REPO_ROOT / "results" / "galaxyppg_hr_external_replication_stage2.json"
OUT_PATH = REPO_ROOT / "results" / "galaxyppg_hr_full_grouped_cv_stage3.json"
...
def run_fold(fold_idx: int, folds: list[list[str]]) -> dict:
    ...
    for seed in SEEDS:
        ...
        for name, model in (("A_cap", model_a), ("B", model_b), ("C", model_c)):
            ckpt_path = CKPT_DIR / f"galaxyppg_hr_fullcv_fold{fold_idx}_{name}_seed{seed}.pt"
...
def main() -> None:
    ...
    per_subject_single = json.loads((REPO_ROOT / "results" / "galaxyppg_hr_per_subject_stage2.json").read_text())
