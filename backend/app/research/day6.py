...
def build_readiness() -> ParetoReadinessDay6:
    return ParetoReadinessDay6(
        ...
        blockers=["Scientific benefit is validated for two HR additions and one sleep-stage addition (EOG), but those experiments use different datasets, populations, baselines, metrics (MAE vs macro-F1), and model families and are not comparable or rankable against each other.", "Deployable average power/energy is absent for optical sensing, IMU at the scientifically compatible rate, BioZ, EEG, and the second PPG site.", "Component, PCB/module, and finished wearable masses are unquantified for every component.", "Electrode montages and shared-electrode allocation are unresolved for ECG, thoracic BioZ, EEG, EOG, and leg BioZ.", "Final MCU, memory, radio, battery, regulator, enclosure, and attachment identities/allocations are open.", "Only heart rate and 5-class sleep stage have frozen marginal-value experiments; HRV, respiration, BP/PAT, workload, fatigue, circadian, and fluid targets are not validated."],
