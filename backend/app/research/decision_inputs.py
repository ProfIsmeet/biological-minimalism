    ...
    return ParetoDecisionInputs(
        ...
        readiness=ParetoReadiness(
            ...
            missing_requirements=[
                ...
                "Interaction effects are only PARTIALLY measured: one predeclared factorial experiment (Sleep-EDF EEG x EOG x Resp, result approximately additive/unresolved) now exists, but it covers a single candidate pair on a single target and does not prove a globally minimal subset. One-component-at-a-time marginal value still dominates the evidence, so a formal Pareto frontier cannot be declared from these experiments alone.",
            ],
