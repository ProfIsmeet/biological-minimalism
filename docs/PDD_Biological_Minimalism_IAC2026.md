...
2. **No model has been trained.** The CNN+Transformer architecture (Sections 10–11) is
   real, implemented, and importable, but has never seen training data. The
   dashboard's live AI Confidence, Fatigue Risk, and all other derived metrics come
   from a transparent, hand-specified rule-based physiology engine (Section 9), whose
   coefficients are engineering choices for this demonstration, not fitted or
