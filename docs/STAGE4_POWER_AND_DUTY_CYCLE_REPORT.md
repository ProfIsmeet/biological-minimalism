...
| `thoracic_bioz` (AD5940-class) | 1.335 mW | `ENGINEERING_ASSUMPTION` | 50 kHz tetrapolar excitation, 4 mA active / 5 µA standby, 100 ms burst per 1 s (10% duty) |
| `leg_bioz` (AD5940-class, separate instance) | 1.335 mW | `ENGINEERING_ASSUMPTION` | Same scenario as thoracic; **EXPERIMENTAL, excluded from base total** (rule 47) |
| `head_afe_eeg_eog` (ADS1299-4-class) | 1.5 mW | `ENGINEERING_ASSUMPTION` | 2 active channels (1 EEG + 1 shared EOG), 150 µA/ch + 300 µA fixed overhead @ 2.5 V (AVDD analog rail only — DVDD 3.3V digital-supply current is excluded, same AFE-only convention as `wrist_ppg`/`ecg_chest`). Day-11 Part-2 explicitly could not verify an exact ADS1299 datasheet power table row in its environment; this environment has the same limitation, so this stays an explicit order-of-magnitude estimate, not a re-labeled datasheet value. |
