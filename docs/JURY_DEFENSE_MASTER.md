...
We verified the training/evaluation stack field-by-field against the frozen manifest (Python 3.13.0, PyTorch 2.6.0+cpu, NumPy 2.5.2, SciPy 1.18.1, scikit-learn 1.9.0, MNE 1.12.1, WFDB 4.3.1, pandas 3.0.5, plus `torch.set_num_threads(4)`) → `RECORDED_ENVIRONMENT_MATCH_FOR_CHECKED_FIELDS`. We scope this deliberately (audit §48): the capture covers interpreter/numerical-backend versions and the thread setting, but NOT the CPU model or the full environment-variable set, so we do not claim a bit-for-bit "EXACT" environment. No package was changed to force a match. (The separate Mac integration env is never used to produce a scientific result.)
...
We can quantify several component and sensing-electronics boundaries — e.g. IMU ~0.018 mW (accel-only), ECG AFE 0.67 mW, TMP117 0.01155 mW, and a wrist sensing-electronics **LED-excluded reference scenario** (datasheet-typical, not a guaranteed bound). But a defensible **system-average** power requires LED timing, EEG/BioZ operating points, MCU/radio behaviour, regulator efficiency, and deployable duty schedules — none of which are frozen. So we report `SYSTEM_AVERAGE_POWER_NOT_READY` rather than a fabricated total. Refusing to invent that number is a strength, not a gap in rigor.
